...
import copy  # Импортируем модуль copy для создания копии объекта
...
class Card:
    ...
    def __copy__(self):
        copied_card = Card()  # Создаем новый объект карты
        copied_card.__data = copy.deepcopy(self.__data)  # Глубоко копируем данные карты
        return copied_card
...
class Game:
    ...
    def get_user_input(self, player_num, keg):
        return input(f'Игрок {player_num}, у вас нет числа {keg}. Введите n для продолжения: ').lower().strip()
    ...
    def play_round(self) -> int:
        ...
        for i, card in enumerate(self.__player_cards):
            ...
            if keg in card:
                ...
                useranswer = self.get_user_input(i + 1, keg)
                if useranswer != 'n':
